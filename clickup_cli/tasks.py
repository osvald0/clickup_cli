...
def fetch_user_tasks(status_filter=None):
    ...
    all_tasks = []
    while True:
        response = make_request(f"team/{config['CLICKUP_TEAM_ID']}/task", params={
        ...
        if status_filter:
            filtered_tasks = [
                task for task in tasks if task["status"]["status"].lower() in [s.lower() for s in status_filter]
            ]
        else:
            filtered_tasks = [
                task for task in tasks if task["status"]["status"].lower() in ["in progress", "planned"]
            ]
    ...
    if all_tasks:
        ...
        for task in all_tasks:
            status_color = Fore.BLUE if task["status"]["status"].lower() == "planned" else Fore.GREEN
            print(f"- {status_color}[{task['status']['status']}] {Fore.YELLOW}{task['name']} (ID: {task['id']}){Style.RESET_ALL}")
            print(f"  {Fore.MAGENTA}URL: {task['url']}{Style.RESET_ALL}")
            print(f"  {Fore.CYAN}Due Date: {format_date(task.get('due_date'))}{Style.RESET_ALL}\n")
    else:
